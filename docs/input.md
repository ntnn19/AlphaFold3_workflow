...
af3_flags:
  ...
  models_dir: /path/to/model_params   # Required: AF3 parameters
  databases_dir: /path/to/public_databases   Required: AF3 databases
...
> **`sample_id` must equal the filename stem of `file`, and the `name` field inside the JSON must match `sample_id`**.For example, if your JSON file is named fold_input.json, then the corresponding `sample_id` and `name` field inside the JSON must be set to fold_input. Both constraints are satisfied automatically when reusing the auto-generated sheet at `<output_dir>/rule_PREPROCESSING/metadata/data_pipeline_samples.tsv`.
...
> **`sample_id` must equal the filename stem of `file`, and the `name` field inside the JSON must match `sample_id`**. Both are satisfied automatically when reusing the auto-generated sheet at `<output_dir>/rule_PREPROCESSING/metadata/inference_samples.tsv`.
...
> **`sample_id` must equal the filename stem of `multimer_file`** (i.e. `Path(multimer_file).stem`). The `name` field inside the multimer JSON must also match `sample_id`. Both are satisfied automatically when reusing the auto-generated sheet at `<output_dir>/rule_PREPROCESSING/metadata/inference_to_data_pipeline_map.tsv`.
